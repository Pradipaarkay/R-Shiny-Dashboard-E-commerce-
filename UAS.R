...
ui <- dashboardPage(skin = "purple",
  ...
  dashboardSidebar(
    sidebarMenu(
      menuItem("Descriptive Data", tabName = "desc_data"),
      menuItem("Variable Relationship", tabName = "var_relation"),
      menuItem("Prediction Dashboard", tabName = "prediction")
    )
  ),
  ...
  dashboardBody(
    tabItems(
      tabItem(
        tabName = "desc_data",
        fluidPage(
          titlePanel(title = div("Descriptive Data", style = "color: #333333; font-size: 40px; font-weight: bold; text-align: center; height: 120px")),
          mainPanel(
      ...
      tabItem(
        tabName = "var_relation",
        fluidPage(
          titlePanel(title = div("Variable Relationship Analysis Dashboard", style = "color: #333333; font-size: 40px; font-weight: bold; text-align: center; height: 120px")),
          sidebarLayout(
            sidebarPanel(
...
server <- function(input, output) {
  ...
  observeEvent(input$plot, {
    ...
    output$scatter_plot <- renderPlot({
      ...
      grid.arrange(scatter_plot, nrow = 2)
  ...
  observeEvent(input$analyze_button, {
    ...
    output$sales_plot <- renderPlot({
      ...
      plot(data$x1, data$y, col = "blue", xlab = "Number of Website Visitors", ylab = "Monthly Sales Volume (in thousands of USD)", main = "Scatterplot of Predicted Monthly Sales Volume", xlim = c(120000, 300000), ylim = c(0, 500))
